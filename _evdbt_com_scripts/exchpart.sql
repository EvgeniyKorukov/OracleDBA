 ...
 *	in_prefix	xxx
 *	in_day		date value of partitions to be shuffled
 *	in_must_create	value is FALSE if no error will be thrown if a
 *                      table with the same name as the proposed temporary
 *                      scratch table's name already exists, otherwise
 *                      default is TRUE if an error should be thrown
 *                      should the temporary scratch table already exist
...
as
	...
	procedure prepare
		(in_owner in varchar2,
		 ...
		 in_day in date,
		 in_must_create in boolean default TRUE,
...
is
	...
	select		high_value,
			...
			tablespace_name,
			partition_position,
                        decode(compression,'ENABLED',' COMPRESS ','') compression,
                        decode(logging,'YES','',' NOLOGGING ') logging
...
if v_subpartitioning_type = 'HASH' then
	...
	--
	/*
	 * Not yet fully implemented - no logic for specifying individual partitions
	 */
	raise_application_error(-20000, 'Not yet implemented for LIST sub-partitioned tables');
/*
	v_errcontext :=
		'create table '||in_prefix||in_table||
		' tablespace '||v_tablespace_name||' parallel '||in_degree||
...
*/
	--
elsif v_subpartitioning_type = 'RANGE' then
	--
	/*
	 * Not yet fully implemented - no logic for specifying individual partitions
	 */
	raise_application_error(-20000, 'Not yet implemented for RANGE sub-partitioned tables');
/*
	v_errcontext :=
		'create table '||in_prefix||in_table||
		' tablespace '||v_tablespace_name||' parallel '||in_degree||
...
if in_debug = FALSE then
	--
	/*
	 * Create the temporary scratch table.  If a table by the same name already
	 * exists, then throw an exception if the input parameter IN_MUST_CREATE is
	 * set to false (zero)...
	 */
	declare
	...
	exception
		when already_exists then
			if in_must_create = FALSE then
				raise_application_error(-20000, 'Table "'||in_prefix||in_table||'" already exists; aborting...');
			end if;
	end;
	--
	/*
...
--
/*
 * Gather table and column statistics on the newly-exchanged partition...
 */
v_errcontext := 'begin dbms_stats.gather_table_stats('||
		...
		' method_opt=>''FOR ALL COLUMNS SIZE ALL''); end;';
dbms_application_info.set_action(v_errcontext);
dbms_output.put_line('SQL: '||v_errcontext);
if in_debug = FALSE then
	execute immediate v_errcontext;
...
is
begin
	return '$Header: 0.1 18May2004, Tim Gorman (Evergreen Database Technologies, Inc.)';
end version;
--
end exchpart;
/
